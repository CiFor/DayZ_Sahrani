...
_newUnit 	= _group createUnit [_model,[18192.828,3124.3936,0],[],0,"NONE"];
sleep 0.1;
...
_mydamage_eh1 = _newUnit addeventhandler ["HandleDamage",{ _this call bot_damageHandler; }];

diag_log format["DEBUG: Player %1 [%2] spawned a Bot: %3",_playerName,_playerID,botPlayers];

_isDead = _newUnit getVariable["USEC_isDead",false];
_doLoop = 0;
diag_log format["DEBUG: START _doLoop %1,_isDead=%2",_doLoop,_isDead];

while {_doLoop < 60 && !_isDead && (_playerID in botPlayers)} do {
	_isDead = _newUnit getVariable["USEC_isDead",false];
	_doLoop = _doLoop + 1;
	sleep 1;
};
...
if (!_isDead) then {
	...
	_playerBackp = [_newBackpackType,_backpackWpn,_backpackMag];
...
botPlayers = botPlayers - [_playerID];
diag_log format["DEBUG: Players %1 [%2] Bot just despawned: %3",_playerName,_playerID,botPlayers];
