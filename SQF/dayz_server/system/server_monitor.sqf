...
nul = [
				...
				(15*60),		//Fixed-Time (in seconds) between each start of a new Chopper
				2,					//Random time (in seconds) added between each start of a new Chopper
				...
				3,					//Amount of Random-Waypoints the Heli gets before he flys to his Point-Of-Crash (using Static-Crashpoint-Coordinates if its enabled)
				0.05,				//Amount of Damage the Heli has to get while in-air to explode before the POC. (0.0001 = Insta-Explode when any damage//bullethit, 1 = Only Explode when completly damaged)
				true,				//Spawn the first Heli right on Server-Start?
				8,					//Maximum Number of Crashsites on the Server, after this amount of Crashsites the Server stops spawning new Helicrashs
				80,					//Percentage chance of each helicrash to send the SOS to all players that own a radio
				5000				//Distance in meters in which all players with Radios will see the SOS-Message
