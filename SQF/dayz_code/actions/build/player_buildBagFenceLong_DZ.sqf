...
if (!_isOk) then {
	_objectTemp = createVehicle ["BagFenceLong_DZ", _location, [], 0, "CAN_COLLIDE"];
	...
	sleep 5;
	//place tent (local)
	_tent = createVehicle ["BagFenceLong_DZ", _location, [], 0, "CAN_COLLIDE"];
	_tent setdir _dir;
	...
	dayzPublishObj = [dayz_characterID,_tent,[_dir,_location],"BagFenceLong_DZ"];
	publicVariable "dayzPublishObj";
	if (isServer) then {
		dayzPublishObj call server_publishObj;
	};
	
	cutText ["A Bag Fence Long has been built !", "PLAIN DOWN"];
} else {
