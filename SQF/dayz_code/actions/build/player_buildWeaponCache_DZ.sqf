...
//_isOk = true;
_classname = "WeaponCache_DZ";
diag_log ("Build Weapon Cache: " + str(_isok) );

_config = configFile >> "CfgMagazines" >> _item;
_text = getText (_config >> "displayName");

if (!_hasToolBox) exitWith {cutText ["\n\nYou need a toolbox !","PLAIN DOWN"]};
if (!_hasCrowBar) exitWith {cutText ["\n\nYou need a crowbar !","PLAIN DOWN"]};
if (_haswoodpile < 4 || _hasNails < 4) exitWith {cutText ["\n\nYou need 4 wood piles and 4 boxes of nails !","PLAIN DOWN"]};
...
diag_log ("Build Weapon Cache surface: " + str(_isok) );
...
if (!_isOk) then {
	...
	cutText ["\n\nYou cant build here !", "PLAIN DOWN"];
};
